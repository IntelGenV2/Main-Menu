...
class Program:
    ...
    def execute(self):
        """Execute the program with timeout protection"""
        ...
        if len(line_numbers) > 0:
            ...
            statement_count = 0
            max_statements = 10000  # Maximum statements to execute
            ...
            while True:
                ...
                if time.time() - start_time > max_execution_time:
                    raise RuntimeError(f"Program execution timed out after {max_execution_time} seconds")
                
                # Check for infinite loop (too many statements)
                statement_count += 1
                if statement_count > max_statements:
                    raise RuntimeError(f"Program execution stopped: too many statements ({max_statements})")
